...
%   Detailed explanation goes here]
	...
	if numel(params)>1
		...
		[ x_min, x_max, y_min, y_max ] = windowed_centroid_bounds( x_ctr, y_ctr ,2 , size(I_grey,2), size(I_grey,1));
		...
		for i=1:N
			...
			[observed_val_part,observed_var_part,Pwx,Pwy]=window_centroid(centroid , 2, I_grey,I_stats );
			[predicted_val_part,predicted_H_part]=pixelval_and_jacobian( [params(3*(i-1)+(1:3)) psf_radius], Pwx(:), Pwy(:));
